export const Counter = {
  ...
  init(options = {}) {
    ...
    this.options.transitionDuration = Number(
      options.transitionDuration ?? this.options.default.transitionDuration
    );
    //assegna transitionTimingFunction
    this.options.transitionTimingFunction =
      options.transitionTimingFunction ??
      this.options.default.transitionTimingFunction;
    //assegna transitionClass
    this.options.transitionClass =
      options.transitionClass ?? this.options.default.transitionClass;
    //assegna starting number
    this.options.start = Number(options.start ?? this.options.default.start);
    //assegna negative numbers allow
    this.options.allowNegativeNumbers =
      typeof options.allowNegativeNumbers === "boolean"
        ? options.allowNegativeNumbers
        : this.options.default.allowNegativeNumbers;
    //assegna fake active class
    this.options.fakeActiveClass =
      options.fakeActiveClass ?? this.options.default.fakeActiveClass;
  ...
  setDisplay() {
    if (this.display === null) {
      ...
      try {
        ...
        console.log(
          "You need to assign " + selector + " to your counter element"
        );
        return;
      }
      // starting number
  ...
  keyboardAction(e) {
    ...
    switch (action) {
      ...
      case "Spacebar":
      case " ":
        //per evitare conflitto con eventuale focus sui pulsanti
        e.preventDefault();
        // simulo cambio stato su checkbox
        let checkbox = document.querySelector(
          '[data-action="toggle-negative"]'
        );
        ...
        } else checkbox.fireEvent("onchange");
        break;
      default:
  ...
  wheelAction(e) {
    e.preventDefault();
    let action =
      e.deltaY < 10 ? "increment" : e.deltaY > 10 ? "decrement" : null;
  ...
  fakeActive(action) {
    ...
    try {
      btn.classList.toggle(this.options.fakeActiveClass);
